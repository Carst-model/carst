...
class FunctionContainer(UserDict):
    ...
    _INTERPOLATION_FUNCS = {
        ...
        lambda funcs, options: (options["diff_coeff"] * ((2. / sqrt(2. * math.pi)) * exp(-0.5 * ((funcs[
            carst_funcs.depth]-5.0)/10.0)**2))),
        carst_funcs.light_attenuation:
        lambda funcs, options: (1.0/(1.+exp(-2*funcs[carst_funcs.depth]*25.0))* exp(-1.0*funcs[carst_funcs.depth]/10.)),
